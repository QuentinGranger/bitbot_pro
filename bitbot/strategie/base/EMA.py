"""
Module pour les stratégies basées sur les moyennes mobiles exponentielles (EMA).

Ce module fournit des fonctions pour calculer les moyennes mobiles exponentielles (EMA)
...
class EMAIndicator:
    """
    Classe pour calculer et analyser les moyennes mobiles exponentielles (EMA)
    sur différentes périodes.
    """
    
    def __init__(self, clean_data: bool = True):
        """
        Initialise la classe des indicateurs de moyennes mobiles exponentielles.
        ...
        self.ema_periods = [9, 12, 26, 50, 100, 200]  # Périodes standard pour EMA
        
    def set_custom_periods(self, ema_periods: List[int] = None) -> None:
        """
        ...
        """
        if ema_periods:
            self.ema_periods = sorted(ema_periods)
            logger.info(f"Périodes EMA personnalisées définies: {self.ema_periods}")
    
    def calculate_ema(self, data: Union[pd.DataFrame, MarketData], 
                     column: str = 'close', 
                     periods: List[int] = None) -> pd.DataFrame:
        """
        Calcule les moyennes mobiles exponentielles (EMA) pour les périodes spécifiées.
        
        Args:
            data: DataFrame ou MarketData contenant les données de prix
            column: Colonne à utiliser pour le calcul (par défaut 'close')
            periods: Liste des périodes pour lesquelles calculer les EMA
                    (utilise self.ema_periods si None)
        
        Returns:
            DataFrame avec les colonnes EMA ajoutées
        """
    ...
    def calculate_all(self, data: Union[pd.DataFrame, MarketData], 
        ...
        """
        Calcule les EMA pour toutes les périodes configurées.
        ...
        Returns:
            DataFrame avec toutes les colonnes EMA ajoutées
        """
        return self.calculate_ema(data, column)
    ...
    def identify_trend(self, data: Union[pd.DataFrame, MarketData], 
        ...
        if timeframe == TimeFrame.MICRO:
            ema_short, ema_long = 'ema_9', 'ema_26'
        elif timeframe == TimeFrame.MESO:
            ema_short, ema_long = 'ema_26', 'ema_50'
        else:  # MACRO
            ema_short, ema_long = 'ema_50', 'ema_200'
        ...
        ema_short_slope = (df[ema_short].iloc[-1] - df[ema_short].iloc[-6]) / 5
        ema_long_slope = (df[ema_long].iloc[-1] - df[ema_long].iloc[-6]) / 5
        
        # Déterminer la tendance
        # Tendance haussière forte
        if (last_close > last_ema_short > last_ema_long and 
            ema_short_slope > 0 and ema_long_slope > 0):
            return TrendType.STRONG_UPTREND
        
        # Tendance haussière
        elif (last_close > last_ema_short > last_ema_long and 
              ema_short_slope > 0):
            return TrendType.UPTREND
        
        # Tendance haussière faible
        elif (last_close > last_ema_short and ema_short_slope > 0):
        ...
        # Tendance latérale
        elif abs(ema_short_slope) < 0.001:
    ...
    def detect_crossover(self, data: Union[pd.DataFrame, MarketData], 
                        fast_period: int = 12, 
                        slow_period: int = 26) -> Tuple[bool, bool]:
        """
        ...
        df = self.calculate_ema(data, periods=[fast_period, slow_period])
        fast_ma = f'ema_{fast_period}'
        slow_ma = f'ema_{slow_period}'
    ...
    def get_ma_distance(self, data: Union[pd.DataFrame, MarketData], 
        ...
        df = self.calculate_ema(data, periods=[period])
...
class MultiTimeFrameEMA:
    """
    Classe pour l'analyse multi-timeframe utilisant les moyennes mobiles exponentielles.
    """
    ...
    def analyze(self, market_data_dict: Dict[str, MarketData]) -> Dict[str, Dict[TimeFrame, TrendType]]:
        ...
        for timeframe, market_data in market_data_dict.items():
            ...
            for tf in TimeFrame:
                trend = self.ema_indicator.identify_trend(market_data, tf)
    ...
    def get_aligned_indicators(self, market_data_dict: Dict[str, MarketData], 
        ...
        for timeframe, market_data in market_data_dict.items():
            df = self.ema_indicator.calculate_all(market_data)
            dfs[timeframe] = df
