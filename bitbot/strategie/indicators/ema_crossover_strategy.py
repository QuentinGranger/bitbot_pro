"""
Module pour la stratégie basée sur le croisement des moyennes mobiles exponentielles (EMA).

Ce module implémente une stratégie de croisement d'EMA qui:
1. Utilise les croisements d'EMA pour repérer les inversions de tendance à court terme
2. Emploie des croisements rapides (EMA9 vs EMA21) pour détecter les mouvements rapides
...
class EMACrossoverStrategy(StrategyBase):
    ...
    def __init__(self, 
        ...
        self.name = "EMACrossoverStrategy"
        self.description = "Stratégie basée sur le croisement des moyennes mobiles exponentielles"
        ...
        logger.info(f"Stratégie de croisement EMA initialisée: "
                   f"EMA{fast_period} vs EMA{slow_period}, "
    ...
    def analyze(self, data: Union[pd.DataFrame, MarketData]) -> Dict:
        ...
        # Récupérer les dernières valeurs des EMA
        fast_col = f'ema_{self.fast_period}'
        slow_col = f'ema_{self.slow_period}'
        
        fast_ema_current = df[fast_col].iloc[-1]
        slow_ema_current = df[slow_col].iloc[-1]
        fast_ema_prev = df[fast_col].iloc[-2]
        slow_ema_prev = df[slow_col].iloc[-2]
        ...
        golden_cross, death_cross = self.ema_indicator.detect_crossover(
            data, 
        ...
        if golden_cross:
            signal = 1
            result['message'] = f"Croisement haussier: EMA{self.fast_period} > EMA{self.slow_period}"
        elif death_cross:
            signal = -1
            result['message'] = f"Croisement baissier: EMA{self.fast_period} < EMA{self.slow_period}"
        
        # Force du signal
        signal_strength = abs(fast_ema_current - slow_ema_current) / slow_ema_current * 100
    ...
    def __str__(self) -> str:
        ...
        return (f"Stratégie de croisement EMA ({self.name})\n"
                f"- EMA rapide: {self.fast_period}\n"
                f"- EMA lente: {self.slow_period}\n"
