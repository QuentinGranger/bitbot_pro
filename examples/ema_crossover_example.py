#!/usr/bin/env python3
"""
Exemple d'utilisation de la stratégie de croisement EMA avec filtre ATR.

Ce script montre comment utiliser la stratégie de croisement des moyennes mobiles
exponentielles avec un filtre ATR pour éviter les faux signaux en période de faible volatilité.
"""
...
def main():
    """Fonction principale pour tester la stratégie de croisement EMA."""
    logger.info("Test de la stratégie de croisement EMA avec filtre ATR")
    ...
    # Initialiser la stratégie EMA Crossover
    # Paramètres standards: EMA9 vs EMA21, filtre ATR à 0.5%
    ...
    logger.info(f"Signaux filtrés: {raw_signals - valid_signals} ({(raw_signals - valid_signals) / raw_signals * 100:.2f}%)")
    ...
    plt.plot(signals_df.index, signals_df[f'ema_{strategy.fast_period}'], label=f'EMA{strategy.fast_period}')
    plt.plot(signals_df.index, signals_df[f'ema_{strategy.slow_period}'], label=f'EMA{strategy.slow_period}')
    ...
    plt.title(f'Stratégie de croisement EMA pour {symbol}')
    plt.ylabel('Prix')
    plt.legend()
    plt.grid(True)
    ...
    output_dir = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), 'outputs', 'ema_strategy')
    os.makedirs(output_dir, exist_ok=True)
    
    # Enregistrer le graphique
    output_file = os.path.join(output_dir, f'{symbol}_ema_crossover_strategy.png')
