...
# -*- coding: utf-8 -*-
"""
Script pour tester l'agrégateur de signaux sur un scénario de marché haussier.
"""
...
def generate_uptrend_market(days=30, start_price=10000.0, daily_gain=0.02, 
                          volatility=0.015, random_seed=42):
    """
    Générer des données de marché avec une tendance haussière.
    
    Args:
        days: Nombre de jours de données
        start_price: Prix initial
        daily_gain: Gain journalier moyen (pourcentage)
    ...
    for i in range(days):
        # Ajout d'une variation aléatoire autour de la tendance
        daily_return = np.random.normal(daily_gain, volatility)
    ...
    # Générer le volume (tendance croissante avec le prix)
    base_volume = 1000000
    volumes = []
    for i, close in enumerate(closes):
        # Volume croissant avec le prix + fluctuation aléatoire
        vol_factor = close / start_price
...
def run_uptrend_backtest(days=60, plot=False):
    """
    Exécuter un backtest avec un scénario de marché haussier.
    ...
    market_data = generate_uptrend_market(days=days)
    ...
    for idx, row in market_data.iterrows():
        ...
        for signal_type in ['ema_crossover', 'sma_crossover', 'rsi', 'market_sentiment', 
                           'volatility_low', 'whale_transaction', 'exchange_netflow']:
        ...
        # Agréger les signaux
        agg_signal = aggregator.aggregate_signals()
        signals.append(agg_signal)
        
        # Simuler des trades sur les signaux forts
        if agg_signal and agg_signal.score >= 65:  # Signal d'achat fort
            trades.append({
                'date': idx,
                'type': 'buy',
                'price': row['close']
            })
        elif agg_signal and agg_signal.score <= 35:  # Signal de vente fort
            trades.append({
                'date': idx,
                'type': 'sell',
                'price': row['close']
    ...
    sell_signals = sum(1 for s in valid_signals if s.score <= 35)
    neutral_signals = sum(1 for s in valid_signals if 35 < s.score < 65)
    
    print(f"\nNombre total de points d'agrégation: {len(valid_signals)}")
    print(f"Signaux d'achat (score >= 65): {buy_signals}")
    print(f"Signaux de vente (score <= 35): {sell_signals}")
    print(f"Signaux neutres: {neutral_signals}")
    
    return market_data, valid_signals, performance
...
def generate_signals(data):
    ...
    # Simuler des signaux de sentiment du marché (aléatoires pour le test)
    np.random.seed(42)
    # Dans une tendance haussière, les signaux de sentiment sont plutôt haussiers
    data['market_sentiment_signal'] = [
        np.random.uniform(40, 90) for _ in range(len(data))
    ]
    ...
    data.loc[data['volatility'] < 1.5, 'volatility_low_signal'] = 55
    
    # Simuler des signaux de transactions de "baleines"
    data['whale_transaction_signal'] = None
    # Ajouter quelques signaux de transactions significatives à des dates aléatoires
    random_days = np.random.choice(range(len(data)), size=5, replace=False)
    for day in random_days:
        score = np.random.uniform(40, 90)  # Scores aléatoires, plutôt haussiers
        data.iloc[day, data.columns.get_loc('whale_transaction_signal')] = score
    
    # Simuler des signaux de flux de fonds sur les exchanges
    data['exchange_netflow_signal'] = [
        np.random.uniform(40, 80) for _ in range(len(data))
    ]
...
def plot_results(market_data, signals, trades):
    ...
    ax1.set_title('Backtest sur marché haussier')
    ax1.set_ylabel('Prix')
    ax1.legend()
    ax1.grid(True)
    
    # Tracer les scores des signaux agrégés
    signal_dates = [datetime.fromtimestamp(s.timestamp) for s in signals if s is not None]
    signal_scores = [s.score for s in signals if s is not None]
    ...
    ax2.axhline(y=35, color='red', linestyle='--', alpha=0.7, label='Seuil de vente')
...
def main():
    ...
    print("Test de l'agrégateur de signaux sur un marché haussier...")
    
    # Exécuter le backtest
    market_data, signals, performance = run_uptrend_backtest(days=60, plot=True)
